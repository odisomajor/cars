...
export async function POST(request: NextRequest) {
  try {
    ...
    const pricePerDay = formData.get('pricePerDay') ? parseFloat(formData.get('pricePerDay') as string) : null
    ...
    if (!title || !make || !model || !year || !location || !condition || !fuelType || !transmission) {
      return NextResponse.json(
    ...
    if (listingType === 'rental' && !pricePerDay) {
      return NextResponse.json(
        { error: 'Price per day is required for rental listings' },
        { status: 400 }
      )
    }
    ...
    if (listingType === 'rental') {
      ...
      createdListing = await prisma.rentalListing.create({
        data: {
          title,
          description,
          make,
